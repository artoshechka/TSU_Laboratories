...
bigNumber bigNumber::operator/(const bigNumber& divisor) const {
	...
	bigNumber result(result_len + 1);
	result.length = result_len + 1;
...
bigNumber bigNumber::operator%(const bigNumber& divisor) const {
	...
	while (dividend.length > 1 && dividend.coefficients[dividend.length - 1] == 0) {
		dividend.length--;
	}
...
istream& operator>>(istream& in, bigNumber& bNum) {
	...
	while (temp.length > 1 && temp.coefficients[temp.length - 1] == 0) {
		temp.length--;
	}
